...
@st.cache_resource
def _local_graph():
    from graph_builder import get_app_graph
...
with st.sidebar:
    ...
    st.divider()
    try:
        from agents_logic import get_active_llm_label

        st.caption(f"LLM đang dùng: **{get_active_llm_label()}**")
    except Exception as exc:
        st.warning(str(exc))
    st.markdown(
        "`AMR_USE_API=1`: gọi FastAPI (`uvicorn api:app`). "
        ...
        try:
            ...
            if (
                ...
                st.error(
                    ...
                    "Đã sửa trong `agents_logic.py` — hãy **restart Streamlit** (Ctrl+C rồi chạy lại)."
                )
            else:
                st.error(err)
        else:
